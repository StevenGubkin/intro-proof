...
\begin{definition}
		A \index{Set}\textbf{set} is a collection of objects (which we also call \index{Element of a Set}) \textbf{elements}.  If $x$ is an element of $S$ we write $x \in S$ and say ``$x$ is an element of the set $S$''.  We consider two sets $A$ and $B$ to be equal if every element of $A$ is also in $B$, and every element of $B$ is also in $A$. 
	\end{definition}
...
\begin{itemize}
		\item The set of \index{Natural Numbers}\textbf{natural numbers} is the set
...
The \index{parity}\textbf{parity} of an integer is whether it is even or odd.  For instance, we could say that the parity of 8 is even, while the parity of 9 is odd.
